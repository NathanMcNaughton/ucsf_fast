...
Make everything flexible and modular so you can tweak individual parts of the pipeline without having to change the 
whole thing.

1. Get all raw annotations from mdai.
2. Select the annotations that are relevant to the task.
    a. They should be on the image-level now.
    b. Both should be masks.
3. Create images out of the masks.
    a. Save the masks as jpgs using the SOPInstanceUID and frame number as the filename along with _Morison.
    b. Create a dataframe that records positive and negative labels for each image.
4. Create a csv file that maps the image filenames to the masks and labels.
5. Save masks, raw images, and labels to a new directory.


How this will be implemented.

get_annotations_from_mdai(args):
    1. Get all annotations from mdai.
    2. Select the annotations that are relevant to the task.
    3. Return the relevant annotations.
    
convert_masks_to_images_and_labels(annotations):
    1. Create images out of the masks.
    2. Save the masks as jpgs using the SOPInstanceUID and frame number as the filename along with _Morison.
        a. but first check if the mask is already saved.
    3. Create a dataframe that records positive and negative labels for each image.
    4. Return the dataframe.

collect_and_rename_images(annotations, image_dir):
    1. Create a csv file that maps the image filenames to the masks and labels.
    2. Save raw images to new directory with new names.
